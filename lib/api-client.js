  ...
  async request(endpoint, options = {}, retryCount = 0) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Với auth endpoints, luôn throw error để frontend có thể hiển thị message
        const isAuthEndpoint = endpoint.includes('/auth/');
        
        if (isAuthEndpoint) {
          // Auth endpoints: throw error với message từ server
          const errorMessage = data?.message || data?.err || data?.error || `HTTP error! status: ${response.status}`;
          throw new Error(errorMessage);
        }
        
        // Với các endpoints khác: Nếu là 404 hoặc 400 với message, trả về data để frontend xử lý thông báo
        if (response.status === 404 || response.status === 400) {
          // Trả về data với flag để frontend biết đây là thông báo, không phải lỗi nghiêm trọng
