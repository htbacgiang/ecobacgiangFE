...
const getFallbackUrls = () => {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (isLocal) {
      ...
      const localApi2 = 'http://127.0.0.1:5000/api';
      if (primary !== localApi && !fallbacks.includes(localApi)) {
        fallbacks.push(localApi);
      }
      if (primary !== localApi2 && !fallbacks.includes(localApi2)) {
        fallbacks.push(localApi2);
      }
    }
  }
...
class ApiClient {
  ...
  async request(endpoint, options = {}, retryCount = 0) {
    ...
    const baseUrl = retryCount === 0 ? (this.lastWorkingUrl || this.baseURL) : this.lastWorkingUrl;
    ...
    try {
      ...
      const isSlugCheckEndpoint = endpoint === '/products/check-slug';
      ...
      if (data && data.status === 'error') {
        // Auth endpoints hoặc lỗi thiếu token → xử lý đặc biệt (redirect / trả _authError)
        ...
        // Riêng endpoint check slug: không throw lỗi global,
        // mà trả về data để phía FE xử lý như validation bình thường
        if (isSlugCheckEndpoint) {
          return data;
        }

        // Các trường hợp còn lại: throw để hiển thị lỗi chung
